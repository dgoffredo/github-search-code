...
def xml_from_sexpr(sexpr) -> str:
    return ET.tostring(element_from_sexpr(sexpr), encoding='unicode', short_empty_elements=False)
...
def _element_from_list(sexpr, builder: ET.TreeBuilder):
    ...
    if len(rest) and isinstance(rest[0], dict):
        # TODO: consider stringifying attributes values, e.g. for integers
